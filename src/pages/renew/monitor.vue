...
<script>
import { getMonitorProject, getMonitorPriceList } from "../../api/monitor";
...
export default {
  ...
  created() {
    ...
    this.getMonitorProject();
    this.getTeamInfo();
    this.getMonitorPriceList();
  },
  mounted() {
    document.title = "监控续费 - EasyAPI";
  },
  methods: {
    //当前时间
    getItem() {
    ...
    //获取项目监控详情
    getMonitorProject() {
      getMonitorProject(this.projectId).then((res) => {
        if (res.data != null) {
          let obj = res.data;
          ...
          //代表已过期
          if (
            new Date(obj.endTime).getTime() < new Date(this.clockItem).getTime()
          ) {
