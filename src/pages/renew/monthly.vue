  ...
  <div class="Insequence">
    ...
    <div class="Insequence_conter">
      <div class="Insequence_service">
        ...
        <div class="left">
          <div class="Insequence_Price" :class="{eaActive:selectMoney === price.month}" v-for="(price,i) in frequency"
               @click="selectMoneyFn(price.month,price.servicePriceId,price.discount,price.price,price.month)">
            <strong v-if="price.month < 12">{{price.month}}月&nbsp;&nbsp;￥{{ price.price - price.discount}}</strong>
            <p v-if="price.month < 12">￥{{onePrice[i]}}/月</p>
            <strong v-if="price.month >= 12">{{price.month / 12}}年&nbsp;&nbsp;￥{{ price.price -
      ...
      <div class="Insequence_service">
        ...
        <div class="left">
          <div class="payment" :class="{state:assignment==='余额支付'}" @click="stand('余额支付')"
               v-if="balance >= price - discount">
            <span><strong>余额支付(￥{{balance}})</strong></span>
          </div>
          <div class="payment_p" :class="{state:assignment==='余额支付'}" @click="stand('余额支付')"
               v-if="balance < price - discount ">
            <span><strong>余额支付(￥{{balance}})</strong></span>
      ...
      <div class="Insequence_fa">
        <Button v-if="balance >= discount || assignment !== '余额支付'"
        ...
        this.$ajax({
          ...
          if (res.data.code !== 0) {
            ...
            for (var i = 0; i < this.frequency.length; i++) {
              second = ((this.frequency[i].price - this.frequency[i].discount) / this.frequency[i].month).toFixed(4)
