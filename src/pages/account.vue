<template>
  <div class="m-wrapper">
    <section>
      <h3>账户余额</h3>
      <div class="m">
        <p class="amount">{{accountGolbalInfo.balance.toFixed(2)}}<span class="amount-unit">元</span></p>
        ...
        <span class="invoiceQR">
          ...
          <div class="Code-picture">
            <img :src="'http://qr.topscan.com/api.php?text=' + picture " alt="" style="width: 100px;height: 100px">
         </div>
        </span>
      </div>
      ...
      <div class="m">
        ...
        <div class="warn-row grid">
          ...
          <div class="warn-row__v">
            <Input class="warn-input" :disabled="!needMoneyWarn" size="small" v-model="moneyWarnSize"></Input>
            <p class="_unit">元</p>
          </div>
        </div>
      </div>
    ...
    <section>
      ...
      <div class="m">
        <div class="team-info grid">
          <div class="team-icon"><img :src="accountGolbalInfo.team.img+'!icon.jpg'" alt=""></div>
          <div>
        ...
        <p v-if="role=='管理员' || role=='创建人'" class="tip">可以修改团队名称, 团队描述和所属行业; 团队信息会在团队列表和团队主页中显示。</p>
      </div>
      <ea-button v-if="role=='管理员' || role=='创建人'" class="team-btn" text="修改团队信息" :space="true" @click="toTeamChange"/>
    </section>
    <section v-if="role=='创建人'">
      <h3>转让团队</h3>
      <div class="m">
        <p>所有者: {{accountGolbalInfo.team.user.nickname}} &nbsp;&nbsp;{{accountGolbalInfo.team.user.username}}</p>
    ...
    <ea-dialog title="账户充值" :open="recharge" width="600" @visibleChange="rcSDVisible">
      <Row>
        ...
        <Row>
          ...
          <div @click="setRechargeType(1)" :class="['zf-type', {'p-active': rechargeType==1}]">
            <img class="zfb" src="../assets/images/alipay.png" alt="">
          </div>
          </Col>
          <Col span="8" class="type-wrapper">
          <div @click="setRechargeType(2)" :class="['zf-type', {'p-active': rechargeType==2}]">
            <img class="wxzf" src="../assets/images/wxpay.png" alt="">
          </div>
          </Col>
          <Col span="8" class="type-wrapper">
          <div @click="setRechargeType(3)" :class="['zf-type', {'p-active': rechargeType==3}]">
    ...
    <ea-dialog title="API充值" :open="apiRecharge" width="500" @visibleChange="arSDVisible">
      <p class="top-10 bottom-10">为了更好的为您的团队，提供实时充值服务，我们提供了<span class="t-blue">API充值接口.</span></p>
      <Row>
        <Row style="margin-bottom: 30px;">
          ...
          <Col class="bottom-10 top-10" span="21">
            <p>请您，使用支付宝<span class="t-blue">单笔转账到支付宝账户接口</span><br>
              对我们的公司帐号leida@daoqun.net, 进行转账</p>
          </Col>
        </Row>
        <Row>
          <Col class="bottom-10 top-5" span="3">
          ...
          <p>转账成功后，请调用我们的<span class="t-blue">支付宝转账确认接口</span><br>
            如果传递的支付宝订单号正确，则自动为您充值到帐号上。</p>
          </Col>
        </Row>
      </Row>
    </ea-dialog>
    ...
    <ea-dialog title="转让团队" :open="teamTransfer" width="300" @visibleChange="ttSDVisible">
      <Row>
        ...
        <Select v-model="transferMember" size="small">
          <Option v-for="item in members" :value="item.user.username" :key="item.userTeamId">{{ item.user.nickname }}
          </Option>
        </Select>
        </Col>
      </Row>
    ...
    <ea-dialog title="修改团队信息" :open="teamInfoChange" width="450" @visibleChange="tiSDVisible">
      <Row>
        ...
        <Select class="type-select" v-model="teamType" size="small">
          <Option v-for="item in industries" :value="item" :key="item">{{ item }}</Option>
        </Select>
        </Col>
        <Col class="bottom-10 top-5" span="8">
        团队URL:
  ...
  export default {
    ...
    data: function () {
      return {
        ...
        teamUrl: "",
        teamDes: "",
        industries: ['互联网', '新媒体', '技术与服务', '娱乐', '生活服务', '教育', '制造业', '家具建材', '金融服务', '旅行与交通', '通讯', '医疗健康', '文化与设计', '其他'],
    ...
    mounted: function () {
      ...
      sockjs.setAttribute('src', 'https://cdn.bootcss.com/sockjs-client/1.0.0/sockjs.min.js')
      let stomp = document.createElement('script')
      stomp.setAttribute('src', 'https://cdn.bootcss.com/stomp.js/2.3.3/stomp.min.js')
      $('#app').append(sockjs)
      $('#app').append(stomp)
    ...
    methods: {
      ...
      jumpInvoice() {
        ...
        window.open("https://fapiao-user-center-web.easyapi.com/?taxNumber=91320211MA1WML8X6T&accessToken=" + this.Itoken + '&' + 'username=' + encryption.toUpperCase());
      },
      //生成二维码
      QRcode: function () {
        var teamId = this.$store.state.user.team.id
        console.log(teamId)
        let encryption = md5('' + teamId)
        this.picture = "https://fapiao-h5.easyapi.com?taxNumber=91320211MA1WML8X6T&accessToken=" + this.Itoken + '%26username=' + encryption.toUpperCase();
      ...
      aliPay: function () {
        ...
        this.$ajax({
          ...
          if (res.data.code) {
            ...
            this.$Modal.confirm({
              title: '提示',
              content: '<p>请您在新打开的页面上完成充值。<br>充值完成后，根据您的情况点击下面按钮。</p>',
              okText: '充值成功',
              cancelText: '充值失败',
              onOk: () => {
                this.$Modal.remove();
      ...
      wxPay: function () {
        ...
        this.$ajax({
          ...
          if (res.data.code===1) {
            ...
            let socket = new SockJS('https://account.easyapi.com/easyapi-socket');
            let stompClient = Stomp.over(socket);
            stompClient.connect({}, (frame) => {
              console.log('Connected: ' + frame);
              stompClient.subscribe('/topic/wxpay/' + _that.accountGolbalInfo.team.user.id, (message) => {
