  ...
  <div class="m-wrapper grid">
    ...
    <div>
      <p class="icon2-plus-circle new" @click="newMember"></p>
        ...
        <p class="bottom-5">从团队中移除<span class="t-blue">{{members[memberSelect].user.nickname}}</span></p>
        <p class="bottom-5">被移除的成员, 将不能再访问EasyAPI上的项目信息, 但<span class="t-blue">跟他相关的数据不会被删除</span></p>
        <ea-button text="确认删除" size="small" type="red" class="bottom-5" @click="confirmClick"/>

        <p class="bottom-5" v-if="hasInviter">在{{members[memberSelect].addTime | dateFormat}}, &nbsp;由管理员&nbsp;{{inviter}}&nbsp;邀请加入团队</p>
    ...
    >
      ...
      <Form ref="newMemberList" :model="newMemberList" :label-width="0">
        <div>
          <template v-for="(item, index) of newMemberList.items">
            <FormItem
              ...
              :rules="[
                { required: true, message: '成员邮箱不能为空', trigger: 'blur' },
              ...
              <Input
                ...
                placeholder="请输入新成员邮箱">
                  <Select v-model="item.type" slot="append" style="width: 70px" :key="index">
                    <Option v-for="item of permisionList" :value="item" :key="item">{{item}}</Option>
                  </Select>
              </Input>
            </FormItem>
          </template>
        ...
        <div>
          ...
          <Input v-model="newMemberRemark" type="textarea" :autosize="{minRows: 3,maxRows: 6}" placeholder="请把附言写在这里(可选)" class="top-5"/>
      ...
      <ea-button text="保存设置" type="main" class="top-20" @click="confirmNewMember"/>
    </ea-dialog>
  </div>
</template>

<script>
  import {getTeamUserList, setMemberType, memberInvitedInfo, addMember, delMember} from '../../api/api'
  ...
  export default {
    ...
    methods: {
      ...
      delMember: function () {
        ...
        console.log(this.members[this.memberSelect].user.id)
        console.log(this.members[this.memberSelect])
        this.$ajax({
          method: 'delete',
